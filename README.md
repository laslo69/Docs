# Documentation personnelle

🚧 Work in progress 🚧

- [Linux](Linux/README.md)
- [Cisco](Cisco/README.md)

Ce dépôt regroupe ma documentation personnelle réalisé dans le cadre de mon apprentissage des systèmes, des réseaux et de l'administration informatique en autodidacte, pour conserver une trace de mon apprentissage et de disposer d'une base de référence personnelle

Les différents documents et travaux pratiques suivent différentes sources

Cisco : 

site :

- [CCNA v1.1.](https://learningcontent.cisco.com/documents/marketing/exam-topics/200-301-CCNA-v1.1.pdf)
- [Goffinet - cisco](https://cisco.goffinet.org/ccna/)
- [My CCIE journey](https://journey2theccie.wordpress.com/)
- [Reussir son CCNA](https://reussirsonccna.fr/)
- [Finger in the net](https://www.fingerinthenet.com/)
- [Study CCNA](https://study-ccna.com/about/)
- [Study CCNP](https://study-ccnp.com/)
- [Cisco made simple](https://www.ciscomadesimple.be/)
- [U Cisco](https://u.cisco.com/tutorials)
- [Network lab](https://www.networklab.fr/)
- [CCIE Pursuit](https://cciepursuit.wordpress.com/)
- [Cisco ofppt](https://cisco.ofppt.info/)

livre :

- Configurez routeurs et commutateurs, Sébastien ARTU & Aurélien ROUX ( ISBN 978 240 903 1625)
- Documentation Cisco
- Beaucoup de PDF sujets divers

Linux :

site :

- [Programme LPIC-1 v5](https://www.lpi.org/our-certifications/exam-101-102-objectives/)
- [Server world](https://www.server-world.info/en/)
- [Goffinet - linux](https://linux.goffinet.org/)
- [Penguin Gym](https://penguin-gym-linux.com/en/)
- [Linux tricks](https://www.linuxtricks.fr/wiki/windows)
- [Oleks](https://oleks.ca/)
- [RDR-IT](https://rdr-it.com/category/linux/)

livre :

- Linux : Préparation à la certification LPIC-1, Sébastien ROHAUT & Philippe BANQUET ( ISBN 978 240 904 3109)
- Linux : Préparation à la certification LPIC-2, Philippe BANQUET ( ISBN 978 240 903 8303)
- Linux : Administration des systèmes et exploitation des services réseau, Philippe BANQUET ( ISBN 978 240 902 7444)
- Linux : Maitrisez l'administration du système, Sébastien ROHAUT & Philippe BANQUET ( ISBN 978 240 904 3666)
- Manpage
- Beaucoup de PDF sujets divers

## TO-DO

Liste non-exhaustive

### Cisco

#### lab

- GLBP
- HSRP
- Port-security
- Routage statique
- VLAN
- VTP
- DHCPv4

#### Futur ajout

- OSPF
- Spanning-tree
- Routage inter-vlan
- NTP
- VRRP
- NAT
- ACL
- SSH

### Linux

- NFS
- Samba
- SSH
- LVM
- cron
- at
- Gestion processus
- Gestion tâche